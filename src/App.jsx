...
return<div style={{paddingBottom:4}}>
  ...
  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
    ...
    <div style={{...cs,background:`linear-gradient(145deg,${C.card2},${C.card})`,padding:12,position:"relative",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:6,padding:"7px 10px",borderRadius:10,background:`${C.rose}10`,border:`1px solid ${C.rose}22`}}>
        {Ico.arrow(C.rose,12)}
        <div style={{fontSize:10,color:C.roseLight,lineHeight:1.35}}>{coach.takeaway}</div>
      </div>
    </div>
...
return<div>
  {/* Reward Shop Header */}
  <div style={{...cs,background:`linear-gradient(145deg,${C.card2},${C.card})`,border:`1px solid ${C.borderMed}`,marginBottom:12}}>
    <SH icon={Ico.rewards(C.gold,14)} title="Reward Shop" right={null}/>
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:7,marginBottom:12}}>
...
const CONTENT={today:Today,goals:Goals,hoops:Sports,rewards:Rewards,progress:Progress};
...
<style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700;800;900&display=swap'); *{box-sizing:border-box} button,[role="button"]{-webkit-tap-highlight-color:transparent;touch-action:manipulation;user-select:none;appearance:none} input,textarea,select{font-size:16px!important} ::-webkit-scrollbar{display:none} body{margin:0;overflow-x:hidden} input[type="time"]::-webkit-calendar-picker-indicator{filter:invert(.6)} input[type="date"]::-webkit-calendar-picker-indicator{filter:invert(.6)}`}</style>
